...
export function usePeer(
  ...
  const cleanup = useCallback(() => {
    ...
    setRemoteAudioStream(null); setIsConnected(false); setIsConnecting(false); setIceState("idle");
  }, []);
  ...
  const initConnection = useCallback(async (asInitiator: boolean) => {
    try {
      ...
      // Set up data channel listener BEFORE any async ops, so the offer
      // can be received even while we're waiting for audio permissions
      if (!asInitiator) {
  ...
  const handleIncomingOffer = useCallback(async (sdp: RTCSessionDescriptionInit) => {
    ...
    if (!pc) return;
    try {
      const answer = await doHandleOffer(pc, sdp);
  ...
  return {
    remoteBlendshapeRef, remoteAudioStream, isConnected, isConnecting, iceState, error,
