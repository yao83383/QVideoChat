  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center px-4 pt-6 pb-16 gap-8">
      ...
      <div className="w-full max-w-3xl flex items-center justify-between">
        ...
        <span className="text-xs text-neutral-500">我的化身</span>
      </div>
      ...
      <div className="w-full max-w-3xl">
        <div className={`flex items-center gap-4 rounded-2xl bg-gradient-to-br ${selectedEntry.tint} border border-white/10 p-4`}>
          <div className="w-16 h-16 rounded-2xl bg-neutral-900/60 flex items-center justify-center border border-white/10 shrink-0">
            <span className="text-4xl select-none" aria-hidden>{selectedEntry.emoji}</span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-green-400 tracking-wider uppercase font-semibold mb-0.5">使用中</p>
            <p className="text-sm sm:text-base font-semibold truncate">{selectedEntry.name}</p>
            <p className="text-xs text-neutral-300/80 line-clamp-1">{selectedEntry.tagline}</p>
          </div>
        </div>
      </div>
...
function PreviewModal({ entry, isCurrent, onCommit, onDismiss }: PreviewProps) {
  ...
  return (
    ...
    >
      <div
        className={`relative w-full max-w-md rounded-3xl bg-gradient-to-br ${entry.tint} border border-white/10 backdrop-blur-md p-5 flex flex-col items-center gap-4 shadow-2xl`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="w-full flex items-center justify-between">
          <div>
            <p className="text-base font-semibold">{entry.name}</p>
            <p className="text-xs text-neutral-400 mt-0.5">{entry.tagline}</p>
          </div>
          <button
            type="button"
            onClick={onDismiss}
        ...
        <p className="text-[10px] text-neutral-500 text-center min-h-[14px]">
